...
extends Animal {
    ...
    private static final EntityDimensions BABY_DIMENSIONS = EntityTypes.PANDA.getDimensions().scale(0.5f).withEyeHeight(0.28125f).withAttachments(EntityAttachments.builder().attach(EntityAttachment.PASSENGER, 0.0f, 0.5625f, 0.0f));
    ...
    public Panda(EntityType<? extends Panda> type, Level level) {
        ...
        this.moveControl = new PandaMoveControl<Panda>(this);
        if (!this.isBaby()) {
    ...
    public @Nullable AgeableMob getBreedOffspring(ServerLevel level, AgeableMob partner) {
        Panda baby = EntityTypes.PANDA.create((Level)level, EntitySpawnReason.BREEDING);
        if (baby != null) {
    ...
    private static class PandaMoveControl<T extends Panda>
    extends MoveControl<T> {
        public PandaMoveControl(T mob) {
            super(mob);
        }
    ...
    extends Goal {
        ...
        public void start() {
            if (this.this$0.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                ...
                if (!items.isEmpty()) {
                    this.this$0.getNavigation().moveTo(items.getFirst(), (double)1.2f);
                }
            } else {
                this.this$0.tryToSit();
            }
