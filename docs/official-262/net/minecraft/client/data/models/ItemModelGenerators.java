...
public class ItemModelGenerators {
    ...
    public static final Identifier TRIM_PREFIX_HELMET = ItemModelGenerators.prefixForSlotTrim("helmet");
    public static final Identifier TRIM_PREFIX_CHESTPLATE = ItemModelGenerators.prefixForSlotTrim("chestplate");
    public static final Identifier TRIM_PREFIX_LEGGINGS = ItemModelGenerators.prefixForSlotTrim("leggings");
    public static final Identifier TRIM_PREFIX_BOOTS = ItemModelGenerators.prefixForSlotTrim("boots");
    public static final List<TrimMaterialData> TRIM_MATERIAL_MODELS = List.of(new TrimMaterialData(MaterialAssetGroup.QUARTZ, TrimMaterials.QUARTZ), new TrimMaterialData(MaterialAssetGroup.IRON, TrimMaterials.IRON), new TrimMaterialData(MaterialAssetGroup.NETHERITE, TrimMaterials.NETHERITE), new TrimMaterialData(MaterialAssetGroup.REDSTONE, TrimMaterials.REDSTONE), new TrimMaterialData(MaterialAssetGroup.COPPER, TrimMaterials.COPPER), new TrimMaterialData(MaterialAssetGroup.GOLD, TrimMaterials.GOLD), new TrimMaterialData(MaterialAssetGroup.EMERALD, TrimMaterials.EMERALD), new TrimMaterialData(MaterialAssetGroup.DIAMOND, TrimMaterials.DIAMOND), new TrimMaterialData(MaterialAssetGroup.LAPIS, TrimMaterials.LAPIS), new TrimMaterialData(MaterialAssetGroup.AMETHYST, TrimMaterials.AMETHYST), new TrimMaterialData(MaterialAssetGroup.RESIN, TrimMaterials.RESIN));
    ...
    private List<RangeSelectItemModel.Entry> createCompassModels(Item compass) {
        ArrayList<RangeSelectItemModel.Entry> overrides = new ArrayList<RangeSelectItemModel.Entry>();
    ...
    private void generateStandardCompassItem(Item compass) {
        ...
        this.itemModelOutput.accept(compass, ItemModelUtils.conditional(ItemModelUtils.hasComponent(DataComponents.LODESTONE_TRACKER), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.LODESTONE), 32.0f, overrides), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.SPAWN), 32.0f, overrides)));
    }

    private void generateRecoveryCompassItem(Item compass) {
        this.itemModelOutput.accept(compass, ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new CompassAngle(true, CompassAngleState.CompassTarget.RECOVERY), 32.0f, this.createCompassModels(compass)));
    }
    ...
    private void generateClockItem(Item clock) {
        ...
        this.itemModelOutput.accept(clock, ItemModelUtils.inOverworld(ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.DAYTIME), 64.0f, overrides), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new Time(true, Time.TimeSource.RANDOM), 64.0f, overrides)));
    }

    private Identifier generateLayeredItem(Item target, Material layer0, Material layer1) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(target, TextureMapping.layered(layer0, layer1), this.modelOutput);
    }

    private Identifier generateLayeredItem(Identifier target, Material layer0, Material layer1) {
        return ModelTemplates.TWO_LAYERED_ITEM.create(target, TextureMapping.layered(layer0, layer1), this.modelOutput);
    }

    private void generateLayeredItem(Identifier target, Material layer0, Material layer1, Material layer2) {
    ...
    private void generateTrimmableItem(Item armor, ResourceKey<EquipmentAsset> equipmentAssetId, Identifier slotTrimPrefix, boolean hasDyedLayer) {
        ...
        ArrayList cases = new ArrayList(TRIM_MATERIAL_MODELS.size());
    ...
    private void generateBow(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.conditional(ItemModelUtils.isUsingItem(), ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new UseDuration(false), 0.05f, pulling0, ItemModelUtils.override(pulling1, 0.65f), ItemModelUtils.override(pulling2, 0.9f)), bowModel));
    }
    ...
    private void generateBrush(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelUtils.rangeSelect((RangeSelectItemModelProperty)new UseCycle(10.0f), 0.1f, base, ItemModelUtils.override(brushing0, 0.25f), ItemModelUtils.override(brushing1, 0.5f), ItemModelUtils.override(brushing2, 0.75f)));
    }
    ...
    private void generateSpyglass(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelGenerators.createFlatModelDispatch(flatModel, inHandModel));
    }
    ...
    private void generateSpear(Item item) {
        ...
        this.itemModelOutput.accept(item, ItemModelGenerators.createFlatModelDispatch(flatModel, inHandModel), new ClientItem.Properties(true, false, 1.95f));
    }
    ...
    public void run() {
        ...
        Items.DYE.forEach(item -> this.generateFlatItem((Item)item, ModelTemplates.FLAT_ITEM));
        ...
        Items.HARNESS.forEach(item -> this.generateFlatItem((Item)item, ModelTemplates.FLAT_ITEM));
