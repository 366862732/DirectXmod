...
public class Sheets {
    ...
    public static final WeatheringCopperCollection.ByState<MultiblockChestResources<SpriteId>> CHEST_COPPER = ChestSpecialRenderer.COPPER.map(r -> r.map(CHEST_MAPPER::apply));
    ...
    public static SpriteId chooseSprite(ChestRenderState.ChestMaterialType materialType, ChestType type) {
        return switch (materialType) {
            ...
            case ChestRenderState.ChestMaterialType.COPPER_UNAFFECTED -> CHEST_COPPER.unaffected().select(type);
            case ChestRenderState.ChestMaterialType.COPPER_EXPOSED -> CHEST_COPPER.exposed().select(type);
            case ChestRenderState.ChestMaterialType.COPPER_WEATHERED -> CHEST_COPPER.weathered().select(type);
            case ChestRenderState.ChestMaterialType.COPPER_OXIDIZED -> CHEST_COPPER.oxidized().select(type);
        };
    }
}
