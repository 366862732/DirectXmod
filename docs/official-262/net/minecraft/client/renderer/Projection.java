...
public class Projection {
    ...
    public Matrix4f getMatrix(Matrix4f dest) {
        ...
        if (this.projectionType == ProjectionType.PERSPECTIVE) {
            return dest.set((Matrix4fc)this.matrix.setPerspective(this.perspectiveFov * ((float)Math.PI / 180), this.width / this.height, near, far, zZeroToOne));
        }
        return dest.set((Matrix4fc)this.matrix.setOrtho(0.0f, this.width, this.orthoInvertY ? this.height : 0.0f, this.orthoInvertY ? 0.0f : this.height, near, far, zZeroToOne));
    }
