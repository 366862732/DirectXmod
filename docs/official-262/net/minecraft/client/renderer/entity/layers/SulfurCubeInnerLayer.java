...
public class SulfurCubeInnerLayer
extends RenderLayer<SulfurCubeRenderState, SulfurCubeModel> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, SulfurCubeRenderState state, float yRot, float xRot) {
        int overlayCoords;
        int n = overlayCoords = state.fuseRemainingTicks > 0.0f && TntRenderer.isLit(state.fuseRemainingTicks) ? OverlayTexture.pack(OverlayTexture.u(1.0f), 10) : LivingEntityRenderer.getOverlayCoords(state, 0.0f);
