...
extends AbstractFriendsTab {
    private static final Component RECEIVED_HEADER = Component.translatable("gui.friends.pending.received").withStyle(ChatFormatting.BOLD, ChatFormatting.UNDERLINE);
    ...
    public void updateEntries(List<IncomingEntry> incomingEntries, List<OutgoingEntry> outgoingEntries) {
        ...
        if (!incomingEntries.isEmpty()) {
            this.pendingScrollableContent.addChild(this.createText(RECEIVED_HEADER, this.screen.getFont(), this.getListContentWidth()), LayoutSettings::alignHorizontallyCenter);
            incomingEntries.forEach(this.pendingScrollableContent::addChild);
        }
        if (!outgoingEntries.isEmpty()) {
            this.pendingScrollableContent.addChild(this.createText(SENT_HEADER, this.screen.getFont(), this.getListContentWidth()), LayoutSettings::alignHorizontallyCenter);
