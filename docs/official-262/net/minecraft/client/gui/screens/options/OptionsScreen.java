...
implements HasGamemasterPermissionReaction {
    ...
    protected void init() {
        ...
        if (this.inWorld) {
            subHeader.addChild(Button.builder(Component.translatable("options.worldOptions.button"), button -> this.minecraft.gui.setScreen(new WorldOptionsScreen(this, Objects.requireNonNull(this.minecraft.level)))).build());
        } else {
    ...
    private LayoutElement createOnlineButton() {
        return Button.builder(Component.translatable("options.online"), button -> this.minecraft.gui.setScreen(new OnlineOptionsScreen(this, this.options))).bounds(this.width / 2 + 5, this.height / 6 - 12 + 24, 150, 20).build();
    }
    ...
    private Button openScreenButton(Component message, Supplier<Screen> screenToScreen) {
        return Button.builder(message, button -> this.minecraft.gui.setScreen((Screen)screenToScreen.get())).build();
    }
