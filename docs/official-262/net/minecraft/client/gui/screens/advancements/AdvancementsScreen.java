...
implements ClientAdvancements.Listener {
    ...
    private void extractInside(GuiGraphicsExtractor graphics) {
        ...
        if (tab == null) {
            ...
            graphics.centeredText(this.font, NO_ADVANCEMENTS_LABEL, midX, this.topPos + 18 + 56 - this.font.lineHeight / 2, -1);
            graphics.centeredText(this.font, VERY_SAD_LABEL, midX, this.topPos + 18 + 113 - this.font.lineHeight, -1);
            return;
        }
    ...
    private void extractTooltips(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (this.tabs.size() > 1) {
            for (AdvancementTab tab : this.tabs.values()) {
                if (!tab.isMouseOver(this.leftPos, this.topPos, mouseX, mouseY)) continue;
