...
implements AutoCloseable {
    ...
    private @Nullable StagedVertexBuffer.Draw previousDraw;

    public GuiRenderer(GuiRenderState renderState, FeatureRenderDispatcher featureRenderDispatcher, List<PictureInPictureRenderer<?>> pictureInPictureRenderers) {
    ...
    private void prepareText() {
        this.renderState.forEachText(text -> {
            ...
            text.ensurePrepared().visit(new Font.GlyphVisitor(){
                final /* synthetic */ GuiRenderer this$0;
                {
                    GuiRenderer guiRenderer = this$0;
                    Objects.requireNonNull(guiRenderer);
                    this.this$0 = guiRenderer;
                }

                @Override
                public void acceptRenderable(TextRenderable renderable) {
                    this.this$0.renderState.addGlyphToCurrentLayer(new GlyphRenderState(pose, renderable, scissor));
                }
            });
        });
    }
    ...
    private void prepareItemElements() {
        ...
        this.renderState.forEachItem(itemState -> {
            ...
            if (slotView != null) {
                this.submitBlitFromItemAtlas((GuiItemRenderState)itemState, slotView);
            }
        });
        if (hasOversizedItems.booleanValue()) {
            this.renderState.forEachItem(itemState -> {
                if (itemState.oversizedItemBounds() != null) {
                    ...
                    OversizedItemRenderer oversizedItemRenderer = this.oversizedItemRenderers.computeIfAbsent(itemStackRenderState.getModelIdentity(), object -> new OversizedItemRenderer());
    ...
    private void submitBlitFromItemAtlas(GuiItemRenderState itemState, GuiItemAtlas.SlotView slotView) {
        this.renderState.addBlitToCurrentLayer(new BlitRenderState(RenderPipelines.GUI_TEXTURED_PREMULTIPLIED_ALPHA, TextureSetup.singleTexture(slotView.textureView(), RenderSystem.getSamplerCache().getRepeat(FilterMode.NEAREST)), (Matrix3x2fc)itemState.pose(), itemState.x(), itemState.y(), itemState.x() + 16, itemState.y() + 16, slotView.u0(), slotView.u1(), slotView.v0(), slotView.v1(), -1, itemState.scissorArea(), null));
    }
