...
public class ProfilerPieChart {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics) {
        ...
        firstLineText = (String)firstLineText + currentNodeNameLines.getFirst();
        int col = -1;
        graphics.text(this.font, (String)firstLineText, left, currentNodeNameTop, -1);
        for (i = 1; i < currentNodeNameLines.size(); ++i) {
            graphics.text(this.font, currentNodeNameLines.get(i), left + 10 + zeroPrefixWidth, currentNodeNameTop + i * this.font.lineHeight, -1);
        }
        ...
        for (i = 0; i < list.size(); ++i) {
            ...
            graphics.text(this.font, (String)msg, left, textY, result.getColor());
            msg = PERCENTAGE_FORMAT.format(result.percentage) + "%";
            graphics.text(this.font, (String)msg, right - 50 - this.font.width((String)msg), textY, result.getColor());
            msg = PERCENTAGE_FORMAT.format(result.globalPercentage) + "%";
            graphics.text(this.font, (String)msg, right - this.font.width((String)msg), textY, result.getColor());
        }
    }
