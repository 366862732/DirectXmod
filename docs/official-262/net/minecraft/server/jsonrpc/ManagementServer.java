...
public class ManagementServer {
    ...
    public boolean scheduleHeartbeat(NotificationManager notificationManager, long period) {
        if (this.heartbeat != null && !this.heartbeat.cancel(true)) {
            LOGGER.warn("The existing heartbeat was not canceled and the new heartbeat of {} seconds has not been applied.", (Object)period);
            return false;
        }
        if (period > 0L) {
    ...
    private void start(final MinecraftApi minecraftApi, final @Nullable SslContext sslContext) {
        ...
        ChannelFuture channel = ((ServerBootstrap)((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().handler((ChannelHandler)new LoggingHandler(LogLevel.DEBUG))).channel(NioServerSocketChannel.class)).childHandler((ChannelHandler)new ChannelInitializer<Channel>(this){
            ...
            protected void initChannel(Channel channel) {
                ...
                pipeline.addLast(new ChannelHandler[]{new HttpServerCodec()}).addLast(new ChannelHandler[]{new HttpObjectAggregator(65536)}).addLast(new ChannelHandler[]{this.this$0.authenticationHandler}).addLast(new ChannelHandler[]{new WebSocketServerProtocolHandler("/")}).addLast(new ChannelHandler[]{new WebSocketFrameAggregator(65536)}).addLast(new ChannelHandler[]{new WebSocketToJsonCodec()}).addLast(new ChannelHandler[]{new JsonToWebSocketEncoder()}).addLast(new ChannelHandler[]{new Connection(channel, this.this$0, minecraftApi, jsonrpcLogger)});
            }
