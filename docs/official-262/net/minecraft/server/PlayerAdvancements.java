...
public class PlayerAdvancements {
    ...
    private void registerListeners(AdvancementHolder holder) {
        ...
        for (Map.Entry<String, Criterion<?>> entry : holder.value().criteria().entrySet()) {
            ...
            this.addListener(entry.getValue(), new TriggerInstanceKey(holder, entry.getKey()));
        }
    }
    ...
    private void unregisterListeners(AdvancementHolder holder) {
        ...
        for (Map.Entry<String, Criterion<?>> entry : holder.value().criteria().entrySet()) {
            ...
            this.removeListener(entry.getValue().trigger(), new TriggerInstanceKey(holder, entry.getKey()));
        }
    }
    ...
    private <T extends CriterionTriggerInstance> Map<TriggerInstanceKey, T> getOrCreateTriggerMapForType(CriterionTrigger<T> type) {
        return this.activeTriggers.computeIfAbsent(type, criterionTrigger -> new HashMap());
    }

    private <T extends CriterionTriggerInstance> void addListener(Criterion<T> typeAndInstance, TriggerInstanceKey criterion) {
        this.getOrCreateTriggerMapForType(typeAndInstance.trigger()).put(criterion, typeAndInstance.triggerInstance());
    }

    public <T extends CriterionTriggerInstance> @Nullable Map<TriggerInstanceKey, T> getTriggerMapForType(CriterionTrigger<T> type) {
        return this.activeTriggers.get(type);
    }

    private <T extends CriterionTriggerInstance> void removeListener(CriterionTrigger<T> type, TriggerInstanceKey criterion) {
