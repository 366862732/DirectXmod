...
public class CommandEncoder {
    ...
    public RenderPass createRenderPass(RenderPassDescriptor descriptor) {
        ...
        if (descriptor.renderArea.x() < 0 || descriptor.renderArea.y() < 0 || descriptor.renderArea.x() + descriptor.renderArea.width() > attachmentWidth || descriptor.renderArea.y() + descriptor.renderArea.height() > attachmentHeight) {
        ...
        for (int i = 0; i < colorAttachmentCount; ++i) {
            ...
            if (colorTexture.getWidth(0) == attachmentWidth && colorTexture.getHeight(0) == attachmentHeight) continue;
        ...
        if (descriptor.depthAttachment != null) {
            ...
            if (depthTexture.getWidth(0) != attachmentWidth || depthTexture.getHeight(0) != attachmentHeight) {
                throw new IllegalArgumentException("Depth texture: size does not match expected attachment size. Is " + depthTexture.getWidth(0) + "x" + depthTexture.getHeight(0) + " expected " + attachmentWidth + "x" + attachmentHeight);
            }
        }
    ...
    public void writeToTexture(GpuTexture destination, NativeImage source, int mipLevel, int depthOrLayer, int destX, int destY) {
        ...
        if (destination.getFormat().componentCount() != source.format().components()) {
            throw new IllegalArgumentException("Destination(" + String.valueOf((Object)destination.getFormat()) + ") texture for NativeImage(" + String.valueOf((Object)source.format()) + ") write must have channel count matching source");
        }
        ...
        if (destX + source.getWidth() > destination.getWidth(mipLevel) || destY + source.getHeight() > destination.getHeight(mipLevel)) {
            throw new IllegalArgumentException("Dest texture (" + source.getWidth() + "x" + source.getHeight() + ") is not large enough to write a rectangle of " + source.getWidth() + "x" + source.getHeight() + " at " + destX + "x" + destY + " (at mip level " + mipLevel + ")");
        }
    ...
    public void writeToTexture(GpuTexture destination, ByteBuffer source, int mipLevel, int depthOrLayer, int destX, int destY, int width, int height) {
        ...
        if (width * height * destination.getFormat().blockSize() > source.remaining()) {
            throw new IllegalArgumentException("Copy would overrun the source buffer (remaining length of " + source.remaining() + ", but copy is " + width + "x" + height + " of format " + String.valueOf((Object)destination.getFormat()) + ")");
        }
    ...
    public void copyBufferToTexture(GpuBufferSlice source, int sourceX, int sourceY, int sourceWidth, int sourceHeight, GpuTexture destination, int destinationX, int destinationY, int copyWidth, int copyHeight, int mipLevel, int arrayLayer) {
        ...
        if ((long)sourceWidth * (long)copyHeight * (long)destination.getFormat().blockSize() > source.length()) {
            throw new IllegalArgumentException("Copy would overrun the source buffer (remaining length of " + source.length() + ", but copy is " + copyWidth + "x" + copyHeight + " of format " + String.valueOf((Object)destination.getFormat()) + ")");
        }
    ...
    public void copyTextureToBuffer(GpuTexture source, GpuBuffer destination, long offset, Runnable callback, int mipLevel, int x, int y, int width, int height) {
        ...
        if ((long)width * (long)height * (long)source.getFormat().blockSize() + offset > destination.size()) {
            throw new IllegalArgumentException("Buffer of size " + destination.size() + " is not large enough to hold " + width + "x" + height + " pixels (" + source.getFormat().blockSize() + " bytes each) starting from offset " + offset);
        }
        if ((source.usage() & 2) == 0) {
