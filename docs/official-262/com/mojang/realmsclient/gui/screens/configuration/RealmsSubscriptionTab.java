...
implements RealmsConfigurationTab {
    ...
    public RealmsSubscriptionTab(RealmsConfigureWorldScreen configurationScreen, Minecraft minecraft, RealmsServer serverData, Subscription subscription) {
        ...
        helper.addChild(SpacerElement.height(2));
        this.deleteButton = helper.addChild(Button.builder(Component.translatable("mco.configure.world.delete.button"), button -> minecraft.gui.setScreen(RealmsPopups.warningPopupScreen(configurationScreen, Component.translatable("mco.configure.world.delete.question.line1"), popup -> this.deleteRealm()))).bounds(0, 0, 200, 20).build());
        helper.addChild(SpacerElement.height(2));
    ...
    private void deleteRealm() {
        RealmsUtil.runAsync(client -> client.deleteRealm(this.serverData.id), RealmsUtil.openScreenAndLogOnFailure(this.configurationScreen::createErrorScreen, "Couldn't delete world")).thenRunAsync(() -> this.minecraft.gui.setScreen(this.configurationScreen.getLastScreen()), this.minecraft);
