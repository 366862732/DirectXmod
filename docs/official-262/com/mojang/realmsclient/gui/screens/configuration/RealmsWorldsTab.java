...
implements RealmsConfigurationTab {
    ...
    public RealmsWorldsTab(RealmsConfigureWorldScreen configurationScreen, Minecraft minecraft, RealmsServer serverData) {
        super(TITLE);
        ...
        this.optionsButton = buttons.addChild(Button.builder(Component.translatable("mco.configure.world.buttons.options"), button -> minecraft.gui.setScreen(new RealmsSlotOptionsScreen(configurationScreen, serverData.slots.get(serverData.activeSlot).copy(), serverData.worldType, serverData.activeSlot))).bounds(0, 0, 150, 20).build());
        this.backupButton = buttons.addChild(Button.builder(Component.translatable("mco.configure.world.backup"), button -> minecraft.gui.setScreen(new RealmsBackupScreen(configurationScreen, serverData.copy(), serverData.activeSlot))).bounds(0, 0, 150, 20).build());
    ...
    private void resetButtonPressed() {
        if (this.isMinigame()) {
            this.minecraft.gui.setScreen(new RealmsSelectWorldTemplateScreen(Component.translatable("mco.template.title.minigame"), this::templateSelectionCallback, RealmsServer.WorldType.MINIGAME, null));
        } else {
    ...
    private void switchToFullSlot(int selectedSlot, RealmsServer serverData) {
        this.minecraft.gui.setScreen(RealmsPopups.infoPopupScreen(this.configurationScreen, Component.translatable("mco.configure.world.slot.switch.question.line1"), popup -> {
            RealmsConfigureWorldScreen newScreen = this.configurationScreen.getNewScreen();
            this.configurationScreen.stateChanged();
            this.minecraft.gui.setScreen(new RealmsLongRunningMcoTaskScreen(newScreen, new SwitchSlotTask(serverData.id, selectedSlot, () -> this.minecraft.execute(() -> this.minecraft.gui.setScreen(newScreen)))));
        }));
    }

    private void switchToEmptySlot(int selectedSlot, RealmsServer serverData) {
        this.minecraft.gui.setScreen(RealmsPopups.infoPopupScreen(this.configurationScreen, Component.translatable("mco.configure.world.slot.switch.question.line1"), popupScreen -> {
            this.configurationScreen.stateChanged();
