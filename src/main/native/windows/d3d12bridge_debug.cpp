...
public:
    ...
    void shutdown() {
        ...
        for (auto& pair : m_textures) {
            if (pair.second.resource) pair.second.resource->Release();
        }
    ...
    int createTexture() {
        ...
        std::cout << "[DX12] Created texture: " << id << std::endl;
        return id;
    }
    ...
    void deleteTexture(int id) {
        ...
        if (it != m_textures.end()) {
            ...
            m_textures.erase(it);
            std::cout << "[DX12] Deleted texture: " << id << std::endl;
        }
    }

    void bindTexture(int target, int id) {
        m_currentTexture = id;
        std::cout << "[DX12] Bind texture: target=" << target << ", id=" << id << std::endl;
    }
    ...
    void uploadTextureData(int id, int width, int height, const void* data) {
        ...
        HRESULT hr = m_device->CreateCommittedResource(&defaultProps, D3D12_HEAP_FLAG_NONE, &texDesc,
        ...
        hr = m_device->CreateCommittedResource(&uploadProps, D3D12_HEAP_FLAG_NONE, &bufferDesc,
        ...
        srcLoc.PlacedFootprint.Footprint.Format = texDesc.Format;
        srcLoc.PlacedFootprint.Footprint.Width = width;
        srcLoc.PlacedFootprint.Footprint.Height = height;
        srcLoc.PlacedFootprint.Footprint.Depth = 1;
        srcLoc.PlacedFootprint.Footprint.RowPitch = width * 4;
        ...
        SAFE_RELEASE(tempCmdList);
        SAFE_RELEASE(tempAllocator);
        ...
        srvHandle.ptr += (id - 1) * m_srvDescriptorSize;
    ...
    int createBuffer() {
        ...
        m_buffers[id] = Buffer{};
        std::cout << "[DX12] Created buffer: " << id << std::endl;
        return id;
    }
    ...
    void deleteBuffer(int id) {
        ...
        if (it != m_buffers.end()) {
            if (it->second.resource) it->second.resource->Release();
            m_buffers.erase(it);
            std::cout << "[DX12] Deleted buffer: " << id << std::endl;
        }
    }

    void bindBuffer(int target, int id) {
        if (target == 34962) m_currentVertexBuffer = id;
        if (target == 34963) m_currentIndexBuffer = id;
        std::cout << "[DX12] Bind buffer: target=" << target << ", id=" << id << std::endl;
    }
    ...
    void uploadBufferData(int id, const void* data, int size) {
        ...
        ID3D12Resource* uploadBuffer = nullptr;
        HRESULT hr = m_device->CreateCommittedResource(&uploadProps, D3D12_HEAP_FLAG_NONE, &bufferDesc,
            D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&uploadBuffer));
        if (FAILED(hr)) return;
        ...
        if (it->second.resource) it->second.resource->Release();
        it->second.resource = uploadBuffer;
        it->second.size = size;
        it->second.gpuAddress = uploadBuffer->GetGPUVirtualAddress();
        it->second.isValid = true;
    ...
    int createProgram() {
        ...
        m_pipelines[id] = Program{};
        std::cout << "[DX12] Created program: " << id << std::endl;
        return id;
    }
    ...
    int createShader(int type) {
        ...
        shader.type = type;
        m_shaders[id] = shader;
        std::cout << "[DX12] Created shader: type=" << type << ", id=" << id << std::endl;
        return id;
    }
    ...
    void shaderSource(int shaderId, const std::string& source) {
        ...
        HRESULT hr = D3DCompile(hlsl.c_str(), hlsl.length(), nullptr, nullptr, nullptr,
                                "main", target, flags, 0, &shaderBlob, &errorBlob);
        
        if (FAILED(hr)) {
            if (errorBlob) {
                std::cout << "[DX12] Shader compile error: " << (char*)errorBlob->GetBufferPointer() << std::endl;
                errorBlob->Release();
            }
            return;
        }
        
        it->second.bytecode.assign((uint8_t*)shaderBlob->GetBufferPointer(),
    ...
    void attachShader(int programId, int shaderId) {
        auto progIt = m_pipelines.find(programId);
        auto shadIt = m_shaders.find(shaderId);
        if (progIt == m_pipelines.end() || shadIt == m_shaders.end()) return;
        
        if (shadIt->second.type == 35633) {
            progIt->second.vsId = shaderId;
        } else {
            progIt->second.fsId = shaderId;
        }
    ...
    void linkProgram(int programId) {
        ...
        auto vsIt = m_shaders.find(it->second.vsId);
        auto fsIt = m_shaders.find(it->second.fsId);
        
        if (vsIt == m_shaders.end() || fsIt == m_shaders.end() ||
            !vsIt->second.compiled || !fsIt->second.compiled) {
            std::cout << "[DX12] Link program " << programId << " failed" << std::endl;
            return;
        }
        ...
        if (SUCCEEDED(hr)) {
            ...
            std::cout << "[DX12] Program " << programId << " linked" << std::endl;
        }
    }
    ...
    void useProgram(int programId) {
        ...
        auto it = m_pipelines.find(programId);
        if (it != m_pipelines.end() && it->second.pso) {
            m_commandList->SetPipelineState(it->second.pso);
        }
        std::cout << "[DX12] Use program: " << programId << std::endl;
    }
    ...
    void drawElements(int mode, int count, int type, long indices) {
        ...
        if (vbufIt != m_buffers.end() && vbufIt->second.resource) {
            ...
            vbv.BufferLocation = vbufIt->second.gpuAddress;
            vbv.SizeInBytes = vbufIt->second.size;
        ...
        if (ibufIt != m_buffers.end() && ibufIt->second.resource) {
            ...
            ibv.BufferLocation = ibufIt->second.gpuAddress;
            ibv.SizeInBytes = ibufIt->second.size;
        ...
        std::cout << "[DX12] drawElements: count=" << count << std::endl;
    }
    ...
    void drawArrays(int mode, int first, int count) {
        ...
        std::cout << "[DX12] drawArrays: count=" << count << std::endl;
    }
...
private:
    ...
    bool initDevice() {
        ...
        if (FAILED(hr)) {
            ...
            if (SUCCEEDED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
                ...
                hr = D3D12CreateDevice(warp, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_device));
                warp->Release();
                factory->Release();
            }
        }
    ...
    bool initSwapChain() {
        ...
        hr = factory->CreateSwapChainForHwnd(m_commandQueue, m_hwnd, &desc, nullptr, nullptr, &swapChain1);
        factory->Release();
        if (FAILED(hr)) return false;
    ...
    bool initRootSignature() {
        ...
        hr = m_device->CreateRootSignature(0, sigBlob->GetBufferPointer(), sigBlob->GetBufferSize(), IID_PPV_ARGS(&m_rootSignature));
        sigBlob->Release();
        if (FAILED(hr)) return false;
    ...
    std::unordered_map<int, Program> m_pipelines;
    
    int m_currentTexture = 0;
    int m_currentVertexBuffer = 0;
    int m_currentIndexBuffer = 0;
    int m_currentProgram = 0;
    
    HWND m_hwnd = nullptr;
    int m_width = 0, m_height = 0;
    float m_clearColor[4] = {0.1f, 0.2f, 0.4f, 1.0f};
    
    int m_nextTextureId = 0;
    int m_nextBufferId = 0;
    int m_nextShaderId = 0;
    int m_nextProgramId = 0;
};
...
JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_bufferData
    ...
    int currentBuf = getRenderer().getCurrentVertexBuffer();
...
JNIEXPORT jint JNICALL Java_com_dx12_client_D3D12Bridge_createProgram
    (JNIEnv* env, jclass clazz) {
    return getRenderer().createProgram();
}

JNIEXPORT jint JNICALL Java_com_dx12_client_D3D12Bridge_createShader
    (JNIEnv* env, jclass clazz, jint type) {
    return getRenderer().createShader(type);
}

JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_shaderSource
    (JNIEnv* env, jclass clazz, jint shader, jstring source) {
    const char* src = env->GetStringUTFChars(source, nullptr);
    getRenderer().shaderSource(shader, src);
    env->ReleaseStringUTFChars(source, src);
}

JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_compileShader
    (JNIEnv* env, jclass clazz, jint shader) {}

JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_attachShader
    (JNIEnv* env, jclass clazz, jint program, jint shader) {
    getRenderer().attachShader(program, shader);
}

JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_linkProgram
    (JNIEnv* env, jclass clazz, jint program) {
    getRenderer().linkProgram(program);
}

JNIEXPORT void JNICALL Java_com_dx12_client_D3D12Bridge_useProgram
    (JNIEnv* env, jclass clazz, jint program) {
    getRenderer().useProgram(program);
}
...
JNIEXPORT jlong JNICALL Java_com_dx12_client_D3D12Bridge_getBufferAddress
    ...
    return buffer ? (jlong)env->GetDirectBufferAddress(buffer) : 0;
}
