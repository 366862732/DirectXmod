# 第一部分：更新完整版 README.md（含新增贡献板块）
# DirectXmod - Minecraft D3D12 渲染模组
> 为 Minecraft 26.1.2 实现的 D3D12 渲染后端，通过覆盖层窗口 + LevelRenderer 拦截 + 全帧 D3D12 渲染实现完整渲染管线。阻止 OpenGL 渲染，仅使用 D3D12 渲染。
...
# 贡献开发者 & 难题征集
## 1. 参与共建说明
本项目存在多项D3D12渲染核心技术卡点，如果你有可行实现方案、代码优化思路，均可参与贡献：
- 仅提供技术方案：在Issues发布提案，标题标注 `[SOLUTION提案]`
- 代码实现修复：Fork仓库提交PR，关联对应难题Issue

完整贡献流程、代码规范、奖励署名规则详见独立文档：[CONTRIBUTING.md](./CONTRIBUTING.md)

## 2. 贡献永久署名激励（三重认可）
1. **源码永久标注**
成功解决对应难题后，在该模块核心代码文件头部添加注释，记录你的GitHub昵称、修复版本、完成日期；
2. **本README贡献名单收录**
所有有效代码/方案贡献者统一在下方表格登记；
3. **MC百科模组官方词条**
模组正式发布上架MC百科后，将你的账号写入Mod「开发团队」栏目，永久标注你负责攻克的渲染功能模块。
...
- 🟡 粒子帧动画、生命周期渐变、深度穿透修复
- 🟡 实体骨骼蒙皮渲染系统
- ❌ 多Pass分层渲染（GUI/3D场景分离）
- ❌ Shader与后处理框架搭建

## 4. 贡献者名单
| GitHub昵称 | 贡献内容 | 对应版本 | 完成日期 |
|------------|----------|----------|----------|
| 项目主作者 | 项目整体架构、D3D12基础窗口、JNI桥接、顶点数据提取、纹理基础框架 | v0.1.0 ~ v0.3.0 | 2026-06-17 ~ 2026-06-19 |

## 5. 交流渠道
所有技术讨论、方案投稿、BUG反馈统一在仓库 GitHub Issues 提交。
